...
function mapJoin(rows: any[], baseAlias: string, baseId: string, map: MapResult2) {
  ...
  if (map.many) {
    ...
    result = mapExtract(rows.find((row) => row[baseAliasId] === baseId), map.alias)
  }
...
export function mapRows(rows: any[], baseAlias: string, maps: MapResult2[]) {
  const base = {}
